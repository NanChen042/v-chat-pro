...
6. **测试聊天**: 使用 Socket.io 客户端连接 `http://localhost:3000` 并发送 `sendMessage` 事件。
...
export class ChatController {
    ...
    @Get('history')
    async getHistory(@Query('user1Id') user1Id: string, @Query('user2Id') user2Id: string) {
        return await this.chatService.getHistory(parseInt(user1Id), parseInt(user2Id));
    }
...
export class ChatService {
    ...
    async getSessions(userId: number): Promise<any[]> {
        ...
        for (const msg of messages) {
            ...
            if (!otherId || sessionsMap.has(otherId)) continue;
